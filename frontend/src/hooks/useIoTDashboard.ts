...
export function useIoTDashboard(deviceId: string = 'default', pollMs: number = POLL_MS) {
  ...
  // Tractor operation interpretation state
  const [interpretation, setInterpretation] = useState<string | null>(null);
  const [stateKey, setStateKey] = useState<string | null>(null);
  const [stateColor, setStateColor] = useState<string | null>(null);
  const [gpsChanged, setGpsChanged] = useState<boolean | null>(null);
  const [ptoRotating, setPtoRotating] = useState<boolean | null>(null);
  const [vibrating, setVibrating] = useState<boolean | null>(null);
  const [signalsAvailable, setSignalsAvailable] = useState<boolean>(false);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setStateKey(data.state_key ?? null);
      setStateColor(data.state_color ?? null);
      setGpsChanged(data.gps_changed ?? null);
      setPtoRotating(data.pto_rotating ?? null);
      setVibrating(data.vibrating ?? null);
      setSignalsAvailable(data.signals_available ?? false);
