...
type PresetForm = {
  preset_depth_cm_min: string;
  preset_depth_cm_max: string;
  preset_speed_kmh_min: string;
  preset_speed_kmh_max: string;
...
function getInitialForm(implement: Implement): PresetForm {
  return {
    preset_depth_cm_min: toFieldValue(implement.preset_depth_cm_min),
    preset_depth_cm_max: toFieldValue(implement.preset_depth_cm_max),
    preset_speed_kmh_min: toFieldValue(implement.preset_speed_kmh_min),
    preset_speed_kmh_max: toFieldValue(implement.preset_speed_kmh_max),
...
export function ConfigurationScreen() {
    ...
    setForms((prev) => ({
      ...
      [implementId]: {
        ...(prev[implementId] ?? {
          ...
          preset_depth_cm_max: '',
          preset_speed_kmh: '',
          preset_speed_kmh_min: '',
          preset_speed_kmh_max: '',
  ...
  const onSave = async (implement: Implement) => {
    ...
    const payload = {
      preset_depth_cm_min: parseOptionalNumber(form.preset_depth_cm_min),
      preset_depth_cm_max: parseOptionalNumber(form.preset_depth_cm_max),
      preset_speed_kmh_min: parseOptionalNumber(form.preset_speed_kmh_min),
      preset_speed_kmh_max: parseOptionalNumber(form.preset_speed_kmh_max),
            ...
            <Card key={implement.id} variant="elevated" spacing="default" style={styles.card}>
              ...
              {expanded ? (
                <View style={styles.formSection}>
                  <RangeField
                    label="DEPTH (CM)"
                    minValue={form.preset_depth_cm_min}
                    maxValue={form.preset_depth_cm_max}
                    onChangeMin={(value) => updateForm(implement.id, 'preset_depth_cm_min', value)}
                    onChangeMax={(value) => updateForm(implement.id, 'preset_depth_cm_max', value)}
                    minPlaceholder="e.g. 15"
                    maxPlaceholder="e.g. 22"
                  />

                  <RangeField
                    label="SPEED (KM/H)"
                    minValue={form.preset_speed_kmh_min}
                    maxValue={form.preset_speed_kmh_max}
                    onChangeMin={(value) => updateForm(implement.id, 'preset_speed_kmh_min', value)}
                    onChangeMax={(value) => updateForm(implement.id, 'preset_speed_kmh_max', value)}
