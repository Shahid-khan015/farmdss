...
# [EXTERNAL-MODEL] ASABE D497 Table 1, columns F1/F2/F3 (fine/medium/coarse).
# The DSS document names the texture classes but does not tabulate them; these
# values match D497 row-for-row -- the same standard Eq. 3.1 itself comes from --
# so they are cited to it rather than left unsourced. Previously tagged LEGACY
# "no cited source"; re-tagged on that evidence, with no numeric change.
# Keyed by ImplementType.value / SoilTexture.value.
