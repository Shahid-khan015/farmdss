...
def rear_ballast_required_kg(
    *,
    ...
    None -- mirroring `front_ballast_required_kg`'s (value, feasible) shape.
    ...
    for _ in range(BALLAST_SOLVER_MAX_ITERATIONS):
        ...
        if mu_prime <= 0:
            return None, (
                "Rear ballast could not be sized: at the 15% target slip the wheel numeric "
                ...
                "soil, or reduce depth/speed/width to lower the draft.".format(bn_prime, r_prime / 2.0)
            )
    ...
    return max(0.0, br_r / GRAVITY), None
