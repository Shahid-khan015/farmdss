...
def calculate_active_passive_performance(inputs: ActivePassiveInputs) -> dict:
    ...
    #
    # A non-positive mu means no net traction develops at the solved slip, so
    # Rreq = Deff/mu is undefined. Reported as unavailable with the reason, the
    # same way the Section 3 solver does -- previously this divided into
    # float("inf") and reported an infinite ballast requirement as if it were a
    # real number.
    if slip_solution.mu > 0:
        r_req_n = d_eff / slip_solution.mu
        ...
        ballast_rear_kg = None
        warnings.append(
            "Rear ballast could not be sized: no net traction develops at the solved slip "
            "(coefficient of traction is non-positive), so the required rear-axle load is "
            "undefined. Work firmer soil, or reduce depth/speed to lower the effective draft."
        )
